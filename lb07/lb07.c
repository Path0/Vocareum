...
int main()
{
  ...
  while(total < 17)
  {
    ...
    calculateTotal(&total, currentCardValue, &num11Aces);
...
void calculateTotal(int *total, int cardValue, int *num11Aces)
{
  if((*total + cardValue > 21) && (*num11Aces != 0)) // if the value would be over 21 and there is an ace
  {
    ...
    *num11Aces--; // the number of usable aces goes down 1
  }
  else
  {
    *total += cardValue;
